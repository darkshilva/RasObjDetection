...
#parser = argparse.Argumen#tParser()
...
if camera_type == 'webcam':
    ...
    #camera = cv2.VideoCapture('acprop_test.mp4')
    #replace with video.mp4 for offline processing
    ret = camera.set(3,IM_WIDTH)
    ret = camera.set(4,IM_HEIGHT)
    
    #Initialize output video
    #frame size has to be the same as original video.

    out_video = cv2.VideoWriter('object_detected_result_webcam.avi', cv2.VideoWriter_fourcc(*'XVID'), 20, (IM_WIDTH, IM_HEIGHT))
    ...
    while(True):
		...
		#write the detected object images
		if ouput_image = 1
            if count != 0: 
